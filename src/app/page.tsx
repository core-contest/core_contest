...
export default function Home() {
  const router = useRouter();
    return (
        <main className={'dark flex flex-col justify-between bg-background min-h-screen px-5'}>
            <div>
                <div className={'mt-5 flex justify-between rounded-full bg-gray-800 opacity-90 px-4 py-3 items-center'}>
                    ...
                    <button
                        ...
                        className={'text-white text-[14px] px-3 py-1 rounded-[16px] bg-gray-600'}>랭킹 확인하기
                    </button>
                </div>
            </div>
            ...
            <div>
                <div className={'flex flex-col justify-center items-center text-white'}>
                    <div className={'text-gray-400 text-[18px]'}>도전 기회 <span className={'text-red-400'}>2번</span> 남음
                    </div>
                    <button
                        className={'mt-[6px] text-white text-[14px] px-3 py-1 rounded-[16px] bg-gray-600'}
                        onClick={() => {
                            router.push('/admin')
                        }}>관리자 페이지</button>
                </div>
                <button className={'mt-5 w-full mb-5 bg-red-400 text-white py-4 rounded-[8px] text-[18px]'}>다시 그리기
                </button>
            </div>
        </main>
    );
