...
const RankPage = () => {
    ...
    const fetchRankList = async () => {
        try {
            ...
            // undefined 값 제거
            const filteredRankList = rankDataList.filter((item: any) => item !== undefined);
        ...
        <main className={'dark bg-background min-h-screen'}>
            <div className={'flex flex-col mx-5'}>
                ...
                <div className={'mt-3 flex justify-between rounded-full bg-gray-800 opacity-90 px-4 py-3 items-center'}>
                    <div className={'flex items-center gap-x-1 text-gray-300'}><FluentEmojiTrophy/>친구들 중 {rankNumber}등이에요</div>
                    <button
                        onClick={() => {
                            router.push('/')
                        }}
                ...
                <div className={'mt-3'}>
                    {rankList[0] && (<div className={'py-4 border-b border-gray-600'}>
                        <div className={'flex items-center gap-x-3'}>
                            <FluentEmoji1stPlaceMedal/>
                            <div>
                                <div className={'text-white text-[14px]'}>{rankList[0]?.name} / {rankList[0]?.ssn}</div>
                                <div className={'text-gray-400 text-[14px]'}>{rankList[0]?.score}점</div>
                            </div>
                        </div>
                    </div>)}
                    {rankList[1] && (<div className={'py-4 border-b border-gray-600'}>
                        <div className={'flex items-center gap-x-3'}>
                            <FluentEmoji2ndPlaceMedal/>
                            <div>
                                <div className={'text-white text-[14px]'}>{rankList[1]?.name} / {rankList[1]?.ssn}</div>
                                <div className={'text-gray-400 text-[14px]'}>{rankList[1]?.score}점</div>
                            </div>
                        </div>
                    </div>)}
                    {rankList[2] && (
                        <div className={'py-4 border-b border-gray-600'}>
                            <div className={'flex items-center gap-x-3'}>
                                ...
                                <div>
                                    <div
                                        className={'text-white text-[14px]'}>{rankList[2]?.name} / {rankList[2]?.ssn}</div>
                                    <div className={'text-gray-400 text-[14px]'}>{rankList[2]?.score}점</div>
                                </div>
                            </div>
                        </div>
                    )}
                    {rankList[3] && rankList.slice(3).map((rank, index) => {
                        return (
                            <div key={rank?.ssn} className={'border-b border-gray-600 px-3 py-4'}>
                                <div className={'flex items-center gap-x-5 text-[18px] text-red-400'}>
                                    {index + 4}
                                    <div>
                                        <div className={'text-white text-[14px]'}>{rank?.name} / {rank?.ssn}</div>
                                        <div className={'text-gray-400 text-[14px]'}>{rank?.score}점</div>
                                    </div>
                                </div>
                            </div>
                        )
